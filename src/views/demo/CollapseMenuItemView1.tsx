...
const organData: Record<string, OrganData> = {
  ...
  heart: {
    ...
    image: "/Heart.png",
    position: { top: "20%", left: "50%", width: "15%" }
  },
  lungs: {
    name: "Lungs",
...
const clickableRegions = [
  ...
  { id: "lungs", organ: "lungs", x: 120, y: 120 },
...
const OrganApp: React.FC = () => {
  ...
  const getInfoBoxPosition = (organ: string) => {
    const region = clickableRegions.find(r => r.organ === organ);
    if (!region) return {};
    return {
      top: `${region.y + 30}px`,
      left: `${region.x + 80}px`,
    ...
    <div className="flex flex-col items-center max-w-4xl mx-auto p-4">
      <div className="relative w-[300px] h-[600px]">
        <img
          ...
          className={`w-full h-full transition-opacity duration-300 ${selectedOrgan ? 'opacity-30' : 'opacity-100'}`}
        />
        ...
        {clickableRegions.map((region) => (
          ...
          >
            <div className={`w-4 h-4 rounded-full transition-all duration-300 hover:scale-125 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 ${
            ...
            {(hoveredOrgan === region.organ || selectedOrgan === region.organ) && (
              <div className="absolute whitespace-nowrap text-white text-xs bg-black rounded px-1 py-0.5" 
        ...
        {selectedOrgan && (
          <div 
            className="absolute bg-white rounded-lg shadow-lg p-3 max-w-[200px] text-sm z-30 border border-gray-200 transition-all duration-300 ease-in-out transform hover:scale-105"
            style={getInfoBoxPosition(selectedOrgan)}
          >
